...
#Bark通知服务Key env export BarkKey
Bark_Key="$BarkKey"
...
function notify_bark(){
    ...
    if [[ "$body" =~ success ]]; then echo "成功"; else echo "失败";fi
}
...
function setSmsRead(){
    ...
    #echo "Response Body:"
    #echo "$body" | jq . 2>/dev/null || echo "$body"
    if [[ "$body" =~ success ]]; then echo "操作成功"; else echo "操作失败"; fi
}
...
function getSmsCapability(){
    ...
    if [ "$runCount" == 0 ]; then echo "容量 $used / $max"; fi
    if [ $(($used + 10 )) -le "$max" ]; then return; fi
    toDelIds=""
...
function init(){
    getLoginStatus
    if [ "$hasLogin" == "true" ]; then getSmsCapability; getSMSMessages; ((runCount++)); else login; fi
    sleep 15
    init
}
if [[ -z "$Bark_Key" ]]; then echo "BarkKey未设置"; else init; fi